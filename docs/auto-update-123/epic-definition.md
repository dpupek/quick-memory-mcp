# Epic 123 – One-Click Auto-Update from the Admin SPA

## Big Idea

Allow an admin to trigger a **non-destructive binary update** of Quick Memory Server directly from the embedded SPA, without touching configuration or data files. The server should:
...
- Admins can:
  ...
  - Apply an update from the SPA, with clear confirmation that config/data are safe and that the service will briefly restart.
- The update pipeline:
...
- Observability:
  - Update attempts (branch, commit, result) are logged in the main log and visible in the SPA.
...
- Arbitrary branch selection from the SPA (initially a single configured release branch).
